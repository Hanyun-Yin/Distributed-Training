...
args,is_mixed_precision="no",seed:int =42
        ...
        for epoch in range(args.num_epochs):
            ...
            for step, batch in enumerate(tqdm(train_loader)):
                #with accelerator.local_main_process_first():
                        ...
                        #use DP to select trainable tensors ----the most important part of the Green Trainer
            ...
            #flops = flops_counter(model, t_fp=t_fp, t_dy=t_dy, t_dw=t_dw)
            t_end = time.time()
        ...
        if accelerator.is_main_process:
            ...
            print(f"Total GreenTrainer Backward GFLOPs: {len(train_loader) * np.sum(total_bp_flops):.2f}")
            print(f"Total GreenTrainer GFLOPs: {len(train_loader) * np.sum(total_bfp_flops):.2f}")
            print(f"Total GreenTrainer GFLOPs Speedup: {np.mean(total_bfp_speedup):.2f}x")
        
        accelerator.end_training()

#normal training_function    
def train(
        args,is_mixed_precision="no",seed:int =42
    ):  
...
#test code
def code_test(
        args,mixed_precision="fp16",seed:int =42
):
    set_seed(seed)
    accelerator = Accelerator(mixed_precision=mixed_precision,log_with="tensorboard", project_dir=args.log_dir)
    config = {
...
if __name__=='__main__':
   ...
   parser.add_argument('--task', type=str, default='train', help='train, evaluate or distributed evaluation')
...
#=========================================================================================================
   ...
   if args.task == 'train':#to train
        ...
        #code_test(args)
        train_green(args)
   elif args.task=='evaluate':#to evaluate using one gpu
        test_loader, tokenizer= dataset_loader(
        dataset_name=args.dataset_name,
        split="test",
        tokenizer_name=args.model_name,
   ...
   else:#to distributedly evaluate 
        test_loader, tokenizer = dataset_loader(
        dataset_name=args.dataset_name,
        split="test",
        tokenizer_name=args.model_name,
